...
export dbpedia_workspace=`pwd`"/$lang_i18n"
...
wget -q --spider $opennlpmodels/$lang_i18n-chunker.bin
if [ $? -eq 0 ] ; then
   wget $opennlpmodels/$lang_i18n-chunker.bin
else
   echo "$lang_i18n"'-chunker.bin not found. Getting an English version...'
   wget $opennlpmodels/en-chunker.bin -O $lang_i18n-chunker.bin
fi

wget -q --spider $opennlpmodels/$lang_i18n-ner-location.bin
if [ $? -eq 0 ] ; then
   wget $opennlpmodels/$lang_i18n-ner-location.bin
else
   echo "$lang_i18n"'-ner-location.bin not found. Getting an English version...'
   wget $opennlpmodels/en-ner-location.bin -O $lang_i18n-ner-location.bin
fi

wget -q --spider $opennlpmodels/$lang_i18n-ner-organization.bin
if [ $? -eq 0 ] ; then
   wget $opennlpmodels/$lang_i18n-ner-organization.bin
else
   echo "$lang_i18n"'-ner-organization.bin not found. Getting an English version...'
   wget $opennlpmodels/en-ner-organization.bin -O $lang_i18n-ner-organization.bin
fi

wget -q --spider $opennlpmodels/$lang_i18n-ner-person.bin
if [ $? -eq 0 ] ; then
   wget $opennlpmodels/$lang_i18n-ner-person.bin
else
   echo "$lang_i18n"'-ner-person.bin not found. Getting an English version...'
   wget $opennlpmodels/en-ner-person.bin -O $lang_i18n-ner-person.bin
fi

wget  -q --spider $opennlpmodels/$lang_i18n-pos-maxent.bin
if [ $? -eq 0 ] ; then
   wget $opennlpmodels/$lang_i18n-pos-maxent.bin
else
   echo "$lang_i18n"'-ner-pos-maxent.bin not found. Getting an English version...'
   wget $opennlpmodels/en-pos-maxent.bin -O $lang_i18n-pos-maxent.bin
fi

wget  -q --spider $opennlpmodels/$lang_i18n-sent.bin
if [ $? -eq 0 ] ; then
   wget $opennlpmodels/$lang_i18n-sent.bin
else
   echo "$lang_i18n"'-sent.bin not found. Getting an English version...'
   wget $opennlpmodels/en-sent.bin -O $lang_i18n-sent.bin
fi

wget  -q --spider $opennlpmodels/$lang_i18n-token.bin
if [ $? -eq 0 ] ; then
   wget $opennlpmodels/$lang_i18n-token.bin
else
   echo "$lang_i18n"'-token.bin not found. Getting an English version...'
   wget $opennlpmodels/en-token.bin -O $lang_i18n-token.bin
fi
...
./generate_indexing_properties.sh>$lang_i18n/indexing.properties
